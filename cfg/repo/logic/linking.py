...
def link(*, path: Path, feature: str, dry_run: bool = False) -> Path:
    ...
    if dry_run:
        ...
        dest = resolve_repo_link_dest(cfg_root=ctx.root, rel_path=Path(path), feature=feature)
        if dest.exists():
            raise CfgError(f"Overlay file already exists: {dest}")
        return dest
    return link_repo_file(cfg_root=ctx.root, repo_root=rr, rel_path=path, feature=feature)
