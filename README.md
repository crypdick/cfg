...
Host apply compares the installed `cfg` version with `main`'s `pyproject.toml` on GitHub,
including during `--dry-run`. If versions differ or the check cannot run, apply stops
before changing the host. Run `uv tool upgrade cfg`, then retry. If cfg was installed
from a local checkout, update that checkout before upgrading.
